...
ARCHITECTURE logic OF ps2keyboard IS
  ...
  BEGIN
    IF(clk'EVENT AND clk = '1') THEN           --rising edge of system clock
      ...
      IF(count_idle = clk_freq/18_000 AND error = '0') THEN  --idle threshold reached and no errors detected
        done <= '1';                                   --set flag that new PS/2 code is available
        DOUT <= ps2_word(9 DOWNTO 1);                      --output new PS/2 code
      ELSE                                                   --PS/2 port active or error detected
        done <= '0';                                   --set flag that PS/2 transaction is in progress
      END IF;
